...
app.get('/keywordlist', async (req, res, next) => {
	...
	const [
		...
		PcSexualRatio,
		MobileSexualRatio,
	] = await Promise.all([getKeywordDetail(keyword, access_token)]);
...
app.get('/manage/keyword', async (req, res, next) => {
	const keyword = req.query.keyword;
	try {
		const [manageResult] = await Promise.all([manageKeyword(keyword)]);
...
app.get('/get/access_token', async (req, res) => {
	...
	await page.goto(
		'https://searchad.naver.com/login?returnUrl=https:%2F%2Fmanage.searchad.naver.com&returnMethod=get'
	);
	...
	const access_token_got = JSON.parse(localStorageData.tokens)[`${customerId}`][
		'bearer'
	];
